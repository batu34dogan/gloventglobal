  ...
  return (
    ...
    >
      ...
      <section id="why" className="relative px-6 py-24 sm:px-10">
        ...
        {/* 3 katmanlı sistem kartı: numaralı badge ile. Bağlantı çizgisi kaldırıldı (yarım/soluk
            görünüyordu) — badge'ler tek başına "sıralı sistem" hissini yeterince veriyor. */}
        <div className="relative mx-auto mt-9 grid max-w-5xl items-stretch gap-6 lg:grid-cols-3">
          {whySteps.map((step, index) => (
            <div
              key={step.number}
              className={`relative flex h-full min-h-[160px] flex-col rounded-xl border p-6 backdrop-blur-sm transition-all duration-300 ${
                index === 1
            ...
            >
              <span
                aria-hidden="true"
                className="absolute left-6 right-6 top-0 h-px bg-gradient-to-r from-blue-400/50 via-blue-400/15 to-transparent"
              />
